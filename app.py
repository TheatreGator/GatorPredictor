...
post_opening_weeks = st.sidebar.number_input(
    ...
    value=6, 
    step=1,
    help="Target post-opening run length for modeling (weeks)."
)
...
with tab2:
    st.subheader("🛠️ Build Custom Full-Run Pacing Model from Uploads")
    st.markdown(f"Upload past show sales reports. Each uploaded show's timeline is **automatically detected, normalized, and stretched** to match your target window (`Week -{pre_sale_weeks}` to `Week +{post_opening_weeks}`), where the ultimate final total at closing equals 1.0 (100%).")
    ...
    if uploaded_historical_files:
                    ...
                    if rev_col:
                        ...
                        if not temp_rev.empty:
                            ...
                            if final_rev > 0:
                                ...
                                # Interpolate onto individual temp dataframe then stretch to target model timeline
        ...
        if rev_curves or tix_curves:
            st.success(f"Successfully processed and normalized historical data from **{len(uploaded_historical_files)}** files!")
