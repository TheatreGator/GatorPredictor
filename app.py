...
st.set_page_config(page_title="PaceCue - Advanced Theatrical Forecaster", layout="wide")

st.title("🎭 PaceCue: Advanced Theatrical Sales & Full-Run Pacing Forecaster")
st.markdown("Forecast revenue, ticket volume, average ticket yields, and multi-scenario confidence bands from pre-sale through season close.")
...
pre_sale_weeks = st.sidebar.number_input(
    ...
    value=36, 
    step=1,
    help="Number of weeks before opening night that pre-sales begin."
)
...
# Helper function to independently stretch pre-opening and post-opening timelines
def stretch_full_lifecycle_curve(pacing_df, target_pre_weeks, target_post_weeks):
    if pacing_df is None:
        return None
    
    orig_min_week = int(pacing_df['Weeks Out'].min())
    orig_max_week = int(pacing_df['Weeks Out'].max())
    
    old_weeks = pacing_df['Weeks Out'].values
    old_pcts = pacing_df['Pacing Pct'].values
    
    pre_mask = old_weeks <= 0
    post_mask = old_weeks >= 0
    
    new_weeks = np.arange(-target_pre_weeks, target_post_weeks + 1)
    new_pcts = np.zeros_like(new_weeks, dtype=float)
    
    for i, w in enumerate(new_weeks):
        if w <= 0:
            if target_pre_weeks > 0 and orig_min_week < 0:
                old_equiv = w * (abs(orig_min_week) / target_pre_weeks)
            else:
                old_equiv = w
            new_pcts[i] = np.interp(old_equiv, old_weeks[pre_mask], old_pcts[pre_mask], left=0.0, right=1.0)
        else:
            if target_post_weeks > 0 and orig_max_week > 0:
                old_equiv = w * (orig_max_week / target_post_weeks)
            else:
                old_equiv = w
            new_pcts[i] = np.interp(old_equiv, old_weeks[post_mask], old_pcts[post_mask], left=0.0, right=1.0)
    ...
    if edited_df is not None and not edited_df.empty:
        ...
        if len(clean_df) < 2:
            ...
            if custom_rev_pacing_df is not None:
                rev_pacing_df = stretch_full_lifecycle_curve(custom_rev_pacing_df, int(pre_sale_weeks), int(post_opening_weeks))
            else:
                rev_pacing_df = pd.DataFrame({"Weeks Out": timeline_weeks, "Pacing Pct": default_rev_curve})

            if custom_tix_pacing_df is not None:
                tix_pacing_df = stretch_full_lifecycle_curve(custom_tix_pacing_df, int(pre_sale_weeks), int(post_opening_weeks))
            else:
            ...
            if st.button("Run Full-Run Pacing Forecast", type="primary"):
                ...
                # Scenario Banding (± 8% historical variance band for floor / ceiling)
                full_timeline_rev['Gross Floor'] = full_timeline_rev['Projected Gross'] * 0.92
                full_timeline_rev['Gross Ceiling'] = full_timeline_rev['Projected Gross'] * 1.08
                ...
                full_timeline_tix['Ticket Floor'] = full_timeline_tix['Projected Tickets'] * 0.93
                full_timeline_tix['Ticket Ceiling'] = full_timeline_tix['Projected Tickets'] * 1.07
                
                # --- Extract Milestone Readouts ---
                ...
                # --- Plots (Revenue with Scenario Bands & Tickets) ---
                col_f1, col_f2 = st.columns(2)
                with col_f1:
                    st.subheader("Revenue Projection & Scenario Bands")
                    fig_r, ax_r = plt.subplots(figsize=(8, 4))
                    ax_r.plot(full_timeline_rev['Weeks Out'], full_timeline_rev['Projected Gross'], color='royalblue', linewidth=2, label="Expected Revenue")
                    ax_r.fill_between(full_timeline_rev['Weeks Out'], full_timeline_rev['Gross Floor'], full_timeline_rev['Gross Ceiling'], color='royalblue', alpha=0.2, label="Scenario Band (±8%)")
                    ax_r.scatter(clean_df['Weeks Out'], clean_df['Cumulative Value'], color='darkblue', zorder=5, label="Actual Revenue")
                    ax_r.axvline(0, color='red', linestyle=':', label='Opening')
                    ax_r.set_title("Cumulative Revenue with Confidence Range")
                ...
                with col_f2:
                    st.subheader("Ticket Projection & Scenario Bands")
                    fig_t, ax_t = plt.subplots(figsize=(8, 4))
                    ax_t.plot(full_timeline_tix['Weeks Out'], full_timeline_tix['Projected Tickets'], color='seagreen', linewidth=2, label="Expected Tickets")
                    ax_t.fill_between(full_timeline_tix['Weeks Out'], full_timeline_tix['Ticket Floor'], full_timeline_tix['Ticket Ceiling'], color='seagreen', alpha=0.2, label="Scenario Band (±7%)")
                    ax_t.scatter(clean_df['Weeks Out'], clean_df['Cumulative Tickets'], color='darkgreen', zorder=5, label="Actual Tickets")
                    ax_t.axvline(0, color='red', linestyle=':', label='Opening')
                    ax_t.set_title("Cumulative Tickets with Confidence Range")
                ...
                # --- Weekly Velocity & ATP Analytics Section ---
                st.markdown("---")
                st.subheader("📈 Weekly Velocity & Average Ticket Price (ATP) Analysis")
                
                analysis_df = clean_df.copy()
                if 'Sales Value' not in analysis_df.columns and 'Cumulative Value' in analysis_df.columns:
                    analysis_df['Sales Value'] = analysis_df['Cumulative Value'].diff().fillna(analysis_df['Cumulative Value'])
                if 'Tickets Sold' not in analysis_df.columns and 'Cumulative Tickets' in analysis_df.columns:
                    analysis_df['Tickets Sold'] = analysis_df['Cumulative Tickets'].diff().fillna(analysis_df['Cumulative Tickets'])
                
                analysis_df['ATP'] = analysis_df['Cumulative Value'] / analysis_df['Cumulative Tickets']
                
                v_col1, v_col2 = st.columns(2)
                with v_col1:
                    st.markdown("##### Weekly Incremental Sales (Velocity)")
                    fig_v, ax_v = plt.subplots(figsize=(7, 3))
                    if 'Sales Value' in analysis_df.columns:
                        ax_v.bar(analysis_df['Weeks Out'], analysis_df['Sales Value'], color='rebeccapurple', alpha=0.7)
                    ax_v.set_title("Incremental Revenue per Week")
                    ax_v.set_xlabel("Weeks Out")
                    ax_v.set_ylabel("Weekly Gross (£)")
                    ax_v.grid(True, linestyle='--', alpha=0.5)
                    st.pyplot(fig_v)
                    
                with v_col2:
                    st.markdown("##### Average Ticket Price (Yield) Trend")
                    fig_a, ax_a = plt.subplots(figsize=(7, 3))
                    ax_a.plot(analysis_df['Weeks Out'], analysis_df['ATP'], color='darkorange', marker='o', linewidth=2)
                    ax_a.set_title("Cumulative ATP (£ / Ticket)")
                    ax_a.set_xlabel("Weeks Out")
                    ax_a.set_ylabel("ATP (£)")
...
# ==========================================
model_timeline_weeks = list(range(-int(pre_sale_weeks), int(post_opening_weeks) + 1))

with tab2:
    st.subheader("🛠️ Build Weighted Custom Full-Run Pacing Model")
    st.markdown(f"Upload past show sales reports. Assign custom weights to each file if certain historical shows are more relevant than others.")
    ...
    if uploaded_historical_files:
        file_weights = {}
        st.markdown("#### Assign Historical Show Weights")
        for f in uploaded_historical_files:
            file_weights[f.name] = st.slider(f"Weight for {f.name}", min_value=0.1, max_value=5.0, value=1.0, step=0.1)
                        ...
                        if not temp_rev.empty:
                            ...
                            if final_rev > 0:
                                ...
                                raw_interp = np.interp(
                                    list(range(int(temp_rev['Weeks Out'].min()), int(temp_rev['Weeks Out'].max()) + 1)),
                                    temp_rev['Weeks Out'],
                                    temp_rev['Rev_Pct'],
                                    left=0.0,
                                    right=1.0
                                ...
                                temp_df = pd.DataFrame({
                                    "Weeks Out": list(range(int(temp_rev['Weeks Out'].min()), int(temp_rev['Weeks Out'].max()) + 1)),
                                    "Pacing Pct": raw_interp
                                })
                                stretched_hist = stretch_full_lifecycle_curve(temp_df, int(pre_sale_weeks), int(post_opening_weeks))
                                if stretched_hist is not None:
                                    rev_curves.append(stretched_hist['Pacing Pct'].values)
                                    weights_list.append(file_weights[file.name])
                    ...
                    if tix_col:
                        ...
                        if not temp_tix.empty:
                            ...
                            if final_tix > 0:
                                ...
                                raw_interp_tix = np.interp(
                                    list(range(int(temp_tix['Weeks Out'].min()), int(temp_tix['Weeks Out'].max()) + 1)),
                                    temp_tix['Weeks Out'],
                                    temp_tix['Tix_Pct'],
                                    left=0.0,
                                    right=1.0
                                )
                                temp_df_tix = pd.DataFrame({
                                    "Weeks Out": list(range(int(temp_tix['Weeks Out'].min()), int(temp_tix['Weeks Out'].max()) + 1)),
                                    "Pacing Pct": raw_interp_tix
                                })
                                stretched_hist_tix = stretch_full_lifecycle_curve(temp_df_tix, int(pre_sale_weeks), int(post_opening_weeks))
                                if stretched_hist_tix is not None:
                                    tix_curves.append(stretched_hist_tix['Pacing Pct'].values)
            except Exception as e:
                st.warning(f"Could not parse file {file.name}: {e}")
                
        if rev_curves or tix_curves:
            st.success(f"Successfully processed historical files with weighted blending!")
            ...
            with col_m1:
                if rev_curves:
                    norm_weights = np.array(weights_list) / sum(weights_list)
                    mean_rev = np.average(rev_curves, axis=0, weights=norm_weights)
                    ...
                    st.subheader("Custom Weighted Revenue Baseline")
                    fig_rm, ax_rm = plt.subplots(figsize=(7, 4))
                    ax_rm.plot(rev_model_df['Weeks Out'], rev_model_df['Pacing Pct'], color='royalblue', linewidth=2.5, label="Weighted Revenue Pacing")
            ...
            with col_m2:
                if tix_curves:
                    mean_tix = np.average(tix_curves, axis=0, weights=norm_weights)
                    ...
                    st.subheader("Custom Weighted Ticket Baseline")
                    fig_tm, ax_tm = plt.subplots(figsize=(7, 4))
                    ax_tm.plot(tix_model_df['Weeks Out'], tix_model_df['Pacing Pct'], color='seagreen', linewidth=2.5, label="Weighted Ticket Pacing")
            ...
            if "revenue_model" in dual_model_payload or "ticket_model" in dual_model_payload:
                ...
                st.markdown("---")
                st.download_button(
                    label="📥 Download Weighted Custom Pacing Model Package (.json)",
                    data=model_json_str,
                    file_name="venue_weighted_pacing_models.json",
                    mime="application/json",
                    type="primary"
                )
        else:
            st.error("Could not extract valid revenue or ticket columns from uploaded files.")
