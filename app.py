...
# Helper function to dynamically stretch/scale pacing curves to match sidebar run length
def stretch_pacing_curve(pacing_df, new_post_opening_weeks):
    ...
    # Map post-opening weeks proportionally to the new run length
    mapped_old_weeks = np.where(
        old_weeks > 0, 
        old_weeks * (new_post_opening_weeks / max(orig_max_week, 1)), 
        old_weeks
    )
    
    new_pcts = np.interp(new_weeks, mapped_old_weeks, old_pcts, left=0.0, right=1.0)
    new_pcts[-1] = 1.0 # Ensure closing week is 100%
    ...
    if edited_df is not None and not edited_df.empty:
        ...
        if len(clean_df) < 2:
            ...
            if st.button("Run Full-Run Pacing Forecast", type="primary"):
                ...
                with metric_col1:
                    st.metric(label="Estimated Final Season Gross (At Closing)", value=f"£{projected_final_gross:,.2f}")
                with metric_col2:
                    st.metric(label="Estimated Final Season Tickets (At Closing)", value=f"{int(projected_final_tix):,}")
